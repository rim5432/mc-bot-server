...
def stale(db_path: Optional[Path] = None, days: int = 90) -> list[Capability]:
    """Capabilities not verified within ``days`` days.

    A missing verified_at counts as stale. Used by the ``capability
    verify`` command to surface what needs re-checking.
    """
    ...
    for cap in repo.list():
        if not cap.verified_at:
            result.append(cap)
            continue
        try:
            verified = _dt.date.fromisoformat(cap.verified_at)
            if verified < cutoff:
                result.append(cap)
        except ValueError:
            result.append(cap)
...
def category_summary(category: str, db_path: Optional[Path] = None) -> dict:
    """Detailed breakdown for one category: counts + capability list."""
    repo = CapabilityRepository(db_path)
    caps = repo.list(category=category)
    by_status = {"shipped": 0, "partial": 0, "gap": 0, "deferred": 0}
    for cap in caps:
        by_status[cap.implementation_status] = by_status.get(cap.implementation_status, 0) + 1
    return {
