...
public final class ProductionWiringGameTests {
    ...
    public static void walksViaCommandAndServerTick(GameTestHelper helper) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                helper.setBlock(new BlockPos(x, GametestRig.FLOOR_Y, z),
                    Blocks.SMOOTH_STONE);
                helper.setBlock(new BlockPos(x, GametestRig.FLOOR_Y + 1, z),
                    Blocks.AIR);
            }
        }

        var server = level.getServer();
        var spawnAbs = helper.absolutePos(
            new BlockPos(3, GametestRig.WALK_Y, 8));
        var source = server.createCommandSourceStack()
            .withLevel(level)
            .withPosition(Vec3.atCenterOf(spawnAbs));
        CellPos goalCell = localToCell(helper,
            new BlockPos(12, GametestRig.WALK_Y, 8));
        String gotoCommand = "bot goto " + goalCell.x() + " "
            + goalCell.y() + " " + goalCell.z() + " 0 300";
        ...
        var structureBox = new AABB(origin.getX(), origin.getY(),
            origin.getZ(), origin.getX() + 16, origin.getY() + 8,
            origin.getZ() + 16);

        helper.startSequence()
            .thenExecute(() -> server.getCommands()
                .performPrefixedCommand(source, "botspawn"))
            .thenExecuteAfter(2, () -> {
                var bodies = level.getEntities(
                    McBotServer.BOT_BODY.get(),
                    b -> structureBox.contains(b.position()));
                check(bodies.size() == 1,
                    "/botspawn must leave exactly one body inside the "
                        + "structure, found " + bodies.size());
                body[0] = bodies.get(0);
            })
            .thenExecute(() -> server.getCommands()
                .performPrefixedCommand(source, gotoCommand))
            // No driveTick anywhere here: the Forge ServerTickEvent
            // subscription is the only thing ticking this pipeline.
            .thenWaitUntil(() ->
                check(reached(body[0], goalCell),
                    "waiting for command-driven arrival, bodyAt="
                        + positionOf(body[0]) + " goal=" + goalCell))
