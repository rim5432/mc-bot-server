...
public final class GauntletGameTests {
    ...
    public static void gauntletEndToEnd(GameTestHelper helper) {
        ...
        CellPos goalCell = localToCell(helper,
            new BlockPos(14, GametestRig.WALK_Y, LANE_Z));
        ...
        // placement offsets.
        int floorAbsY = localToCell(helper,
            new BlockPos(0, GametestRig.FLOOR_Y, LANE_Z)).y();
        int slabExitAbsX = localToCell(helper,
            new BlockPos(7, GametestRig.WALK_Y, LANE_Z)).x();
        int plateauWestAbsX = localToCell(helper,
        ...
        helper.startSequence()
            .thenWaitUntil(driveUntil(rig, () -> {
                ...
                check(crossed(rig, slabExitAbsX)
                    || !mission.isActive(),
                    "waiting: crossing the slab carpet");
            }))
            ...
            .thenWaitUntil(driveUntil(rig, () -> {
                checkBodyOnPlane(rig, floorAbsY);
                check(crossed(rig, dropDoneAbsX) || !mission.isActive(),
                    "waiting: dropping off the plateau");
            }))
            .thenExecuteFor(2, driveOnly(rig))
            .thenWaitUntil(driveUntil(rig, () -> {
                checkBodyOnPlane(rig, floorAbsY);
                check(reached(rig.body(), goalCell)
                    || !mission.isActive(),
                    "waiting: threading the fence gap");
            }))
    ...
    private static void checkBodyOnPlane(GametestRig.Rig rig,
                                         int floorAbsY) {
        check(rig.body().getBlockY() >= floorAbsY,
            "body left the walk plane downward at "
            + positionOf(rig.body()));
    }
    ...
    private static void buildCorridor(GameTestHelper helper) {
        ...
        for (int x = 1; x <= 14; x++) {
            for (int y = 1; y <= 2; y++) {
                helper.setBlock(new BlockPos(x, fy + y, 6),
                    Blocks.SMOOTH_STONE);
                helper.setBlock(new BlockPos(x, fy + y, 10),
                    Blocks.SMOOTH_STONE);
            }
        }
        for (int x = 4; x <= 6; x++) {
            for (int z = 7; z <= 9; z++) {
                helper.setBlock(new BlockPos(x, fy, z),
                    Blocks.SMOOTH_STONE_SLAB);
            }
        }
        for (int x = 8; x <= 10; x++) {
            for (int z = 7; z <= 9; z++) {
                helper.setBlock(new BlockPos(x, fy + 1, z),
                    Blocks.SMOOTH_STONE);
            }
        }
    ...
    private static void assertCompleted(GametestRig.Rig rig) {
        List<String> kinds = rig.events().statusSnapshot(0).events()
            .stream().map(e -> e.kind()).toList();
        check(kinds.contains(EventKind.TASK_COMPLETED),
            "TASK_COMPLETED must reach the stream, got " + kinds);
    }
}
