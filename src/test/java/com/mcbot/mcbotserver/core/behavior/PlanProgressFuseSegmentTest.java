...
class PlanProgressFuseSegmentTest {
    ...
    void waypointAdvanceResetsCriterion3Latch() {
        ...
        cursor.set(List.of(new CellPos(0, 64, 0),
            new CellPos(1, 64, 0), new CellPos(9, 64, 0)));
        ...
        assertTrue(fuse.evaluate(cursor,
            new Vec3(0.5, 64, 0.5), anchor));

        // Reach the corner: index advances to the far waypoint.
        Vec3 atCorner = new Vec3(1.5, 64, 0.5);
        cursor.advance(atCorner);
        assertTrue(fuse.evaluate(cursor, atCorner, anchor),
            "index advance is progress");
