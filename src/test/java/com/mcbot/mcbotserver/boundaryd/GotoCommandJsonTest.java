...
class GotoCommandJsonTest {
    ...
    void numericArgsRoundTrip() {
        String wire = "{\"verb\":\"goto\",\"args\":"
            + "{\"x\":10,\"y\":64,\"z\":-3,\"tolerance\":1}}";
        BotCommand decoded = GotoCommandJson.fromJson(wire);
        assertEquals("goto", decoded.verb());
        assertEquals(Map.of("x", "10", "y", "64", "z", "-3",
            "tolerance", "1"), decoded.args());
    ...
    void emptyArgsRoundTrip() {
        ...
        assertEquals(cmd, GotoCommandJson.fromJson(
            GotoCommandJson.toJson(cmd)));
    }
}
