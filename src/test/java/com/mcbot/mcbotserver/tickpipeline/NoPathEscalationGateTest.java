...
class NoPathEscalationGateTest {
    ...
    private static MoveGraph lineGraph(int extent) {
        return cell -> {
            ...
            if (x < extent && cell.y() == 64 && cell.z() == 0) {
                return List.of(new StubMove(cell,
                    new CellPos(x + 1, 64, 0)));
            }
            return List.of();
        };
    }
    ...
    void drainedUnreachableGoalFailsNoPathSameTick() {
        ...
        ExecutionReport report =
            mover.tick(world, Directive.of(goal), new RecordingActor());
    ...
    void reachableGoalKeepsZeroWitnessLedger() {
        ...
        for (int tick = 0; tick < 60
                && report.status() != ExecutionReport.Status.SUCCESS;
                tick++) {
            ...
            assertEquals("0", mover.keepaliveAttrs(pose[0], goalCell)
                .get("noPathWitnesses"));
        }
