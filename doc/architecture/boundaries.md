...
| 62 | Directed melee strike + the land-game hunt yield face: Intent.Strike(entityId) on the USE channel is the addressed hurt press (ledger 61's InteractEntity sibling; Claim channel table whitelists it), served by StrikeEntityExecutor - UUID resolution, shared 3.0 surface reach, the cone path's line-of-sight + lava discipline (an ungated strike hits through walls: the first engine run's holdsFireWhenSightBlocked caught TARGET_ESCAPED via knockback-shoved NoAi target), and the resolver's full crit-aware damage chain with a self-timed item-derived cooldown (no partial-charge leak; the PathfinderMob body has no vanilla charge ticker). CombatBehavior.tickMelee submits one Strike claim per cooldown window - every directed combat order's victim is addressed by id, passive prey included - while Intent.Use keeps the cone resolver for reflex paths. acquisition.hunt_game lands with it (yield-in-inventory assertion; the attack task retires at the kill, drops scatter past GroundPickup's envelope, and the scenario completes the excursion with a goto through the kill site). Marker repair riding the arc: 4db91ac's archive cleanup left issue-0005/0008 contract markers dangling; repointed to boundaries decisions 24/26 and player-behavior-RE.md section 8 | 2026-09-02 |
| 63 | Standing engine red/flake paydown: walk-exhaustion, grindstone-XP, bowtap-tap - three root causes, all scenario-side, none in device code. Walk/sprint exhaustion: the idle head sweep turned the body during hand-driven walks, so body-relative MOVE walked an arc (path 9.05m vs net 2.20m) and the rate test measured net displacement against path-accrued exhaustion; the shared drive pump now owns ROT at priority 50 (the combat hand-pump doctrine extended to hunger drives; production walkers are safe - MOVE and ROT are a pair) and the scenarios repositioned so a pinned-yaw walk stays on the pad. Grindstone-XP (three stacked causes): Sharpness II yield 6..11 points vs the 7-point level floor is a 1-in-6 no-level lottery (pinned to Sharpness V, 23..45 points); a fixed 20-tick wait raced the pooled batch's unpredictable world entity tick (probes: orbs globally invisible at +20, absorbed 30-60 ticks post-scenario; a 200-tick bare poll passed without the world ticking the body) - the poll now drives the body's own production tick explicitly; the device chain verified end-to-end (award at grind center -> 1.5 scan -> mirror 31 points -> level 3). Bowtap 14.096: full-draw base exactly 6 (crit-0 edge) times the vanilla zombie's 2 armor points (x0.984) = 5.904, health 14.096 vs a bound written for exactly-6 - the target's ARMOR attribute is zeroed (the scenario pins the bow's damage, not the target's armor). Engine evidence: 144/144 at gametest-20260902-043922, first all-green pool since the 09-01 batches | 2026-09-02 |
| 64 | Concurrency story alignment: the gate regex (atomic/locks/Concurrent anchor) never matched ExecutorService/Executors/new Thread, so PlanWorker's single-thread A* pool lived outside ConcurrencyPrimitiveGateTest unregistered while the stated policy said core is single-threaded. Policy of record restated (tick thread owns all bot state; exactly ONE registered thread-bearing file - PlanWorker, completed futures read on the tick thread, anticipated by entry 17's ViewMode) and a second set-equality scan (executor/thread construction forms) pins the thread-bearing file set to exactly PlanWorker.java; EXEMPT_FILES stays empty, the BasicMoves "Executor:" prose collision reworded not exempted | 2026-09-03 |
| 65 | Deep-construction refactor round (user-authorized aggressive): the CPD paydown's three duplication axes paid as structure - gametest fixture/assert families (GametestAsserts split from GametestRig, BotEatingGameTests suite split, scenario budget 18/file trend gate), EventFacts absorbing the fixed-shape taskCancelled/blockBroken/botCrashed factories (wire byte-identical, runtime-pinned; generic parameterized emitters stay by ruling), and VerbWiring(arbiter, events, day, tod) replacing the four-arg handler train (SurvivalInputs precedent). Ratifications riding the arc: BotProcess.onLostControl gains a default no-op (seven identical empty bodies deleted; boundary B semantics unchanged - the arbiter still delivers the callback, parking is never a verdict, resume adjudicates); MissionShell owns the shared named-target absence adjudication + NO_SUCH_ENTITY/TARGET_ESCAPED constants attack/tame duplicated; BotBodyEntity completes the ExperienceMirror/ConsumptionMechanics extractions via public experience()/consumption() accessors. Lint posture PMD/SpotBugs/EP/CPD all zero at HEAD. One wire regression introduced and engine-caught same-day (drink-reflex unequipped reason folded to NOT_EDIBLE; restored per-family NOT_EDIBLE/NO_POTION pair) - the exact class the EventFacts consolidation exists to prevent | 2026-09-03 |
